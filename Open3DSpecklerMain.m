...
global  fullPath nChannels nPlanes imgSize zSlice xyRes zRes chOptions...
        ...
        blueSurfCalTrue blueAffineCalTrue abOptExport sortFittedData...
        sortFittedDataBox
...
function fullPath = importNewData()
    ...
    [file,path] = uigetfile({'*.jpg;*.png;*.tif;*.nd2;*.stk;*.dv;*.sld;*.cif;*.afi;*.svs;*.htd;*.pnl;*.exp;*.spc;*.sdt;*.xml;*.h5;*.ims;*.ch5;*.c01;*.dib;*.vsi;*.wpi;*.r3d;*.rcpnl;*.dcm;*.dicom;*.flex;*.mea;*.res;*.naf;*.his;*.ndpi;*.ndpis;*.vms;*.fff;*.liff;*.obf;*.msr;*.xdce;*.hdr;*.jp2;*.j2k;*.jpf;*.jpk;*.jpx;*.klb;*.fli;*.msr;*.lei;*.lif;*.scn;*.xlef;*.l2d;*.tiff;*.txt;*.mng;*.apl;*.mtb;*.tnb;*.obsep;*.oib;*.oif;*.ome.tiff;*.ome.tf2;*.ome.btf;*.ome.tif;*.ome.tf8;*.ome;*.ome.xml;*.im3;*.cfg;*.spe;*.db;*.tf2;*.tf8;*.btf;*.xys;*.mvd2;*.czi;*.lsm;*.mdb'});
    if file == 0
...
%             particleAnalysisSetup();
        ...
        function commitROI(~,~)
            ...
            switch answer
                case 'Yes'
                    ...
                    cropROI = [floor(xlims{1}(1)) ceil(xlims{1}(2)); floor(ylims{1}(1)) ceil(ylims{1}(2))];
                    updateMainIMG2();
                    if nPlanes > 1
                        try
                            delete(h);
    ...
    function particleAnalysisSetup()
        ...
        global bufferPercXY bufferPercZ sortFittedData sortFittedDataBox
...
%             'String', 'CLEAR CHANNEL DATA', 'FontSize', 7, 'BackgroundColor',[1,0.75,0.75], 'Callback', {@clearCurrentChannel});
       ...
       uicontrol('Parent', mainFig, 'Style', 'pushbutton', 'Position', [viewFigSize(1)*0.045, viewFigSize(2)*0.26, 150, 20],...
        ...
        function gaussButton(~,~)
            ...
            sortFittedData = get(sortFittedDataBox, 'Value');
...
%             end
        ...
        function saveData(~,~)
            ...
            global finalPts488 finalPts594...
                ...
                multi594 multi640 multi405 currentCentroids cropROI xyRes
            
            if ~isempty(cropROI)
                xShift = (cropROI(1,1)-1)*xyRes;
                yShift = (cropROI(2,1)-1)*xyRes;
            else
                xShift = 0;
                yShift = 0;
            end
                ...
                if g2DOpt == 1
                    ...
                    if ~isempty(surfaceOpt) && ~isempty(affineOpt) && (surfaceOpt == 1) && (affineOpt == 0)
                        if (channelCheck(1) == 1) && (channelCheck(2) == 1) && ~isempty(surfX_GR) && ~isempty(surfY_GR)
                            for i = 1:size(currentPts,1)
                                currentPts(i,1) = currentPts(i,1) - surfX_GR(currentPts(i,1)+xShift,currentPts(i,2)+yShift);
                                currentPts(i,2) = currentPts(i,2) - surfY_GR(currentPts(i,1)+xShift,currentPts(i,2)+yShift);
                            end
                            redSurfCalTrue = true;
                            redAffineCalTrue = false;
                    ...
                    elseif ~isempty(surfaceOpt) && ~isempty(affineOpt) && (surfaceOpt == 0) && (affineOpt == 1)
                        if (channelCheck(1) == 1) && (channelCheck(2) == 1) && ~isempty(affineRG) && (affineRG.Dimensionality == 2)
                            currentPts(:,1) = currentPts(:,1)+xShift;
                            currentPts(:,2) = currentPts(:,2)+yShift;
                    ...
                    if ~isempty(surfaceOpt) && ~isempty(affineOpt) && (surfaceOpt == 1) && (affineOpt == 0)
                        if (channelCheck(1) == 1) && (channelCheck(2) == 1) && ~isempty(surfX_GR) && ~isempty(surfY_GR) && ~isempty(surfZ_GR)
                            for i = 1:size(currentPts,1)
                                currentPts(i,1) = currentPts(i,1) - surfX_GR(currentPts(i,1)+xShift,currentPts(i,2)+yShift);
                                currentPts(i,2) = currentPts(i,2) - surfY_GR(currentPts(i,1)+xShift,currentPts(i,2)+yShift);
                                currentPts(i,3) = currentPts(i,3) - surfZ_GR(currentPts(i,1)+xShift,currentPts(i,2)+yShift);
                            end
                            redSurfCalTrue = true;
                            redAffineCalTrue = false;
                ...
                if g2DOpt == 1
                    ...
                    if ~isempty(surfaceOpt) && ~isempty(affineOpt) && (surfaceOpt == 1) && (affineOpt == 0)
                        if (channelCheck(1) == 1) && (channelCheck(3) == 1) && ~isempty(surfX_GFR) && ~isempty(surfY_GFR)
                            for i = 1:size(currentPts,1)
                                currentPts(i,1) = currentPts(i,1) - surfX_GFR(currentPts(i,1)+xShift,currentPts(i,2)+yShift);
                                currentPts(i,2) = currentPts(i,2) - surfY_GFR(currentPts(i,1)+xShift,currentPts(i,2)+yShift);
                            end
                        ...
                        elseif (channelCheck(1) == 0) && (channelCheck(2) == 1) && (channelCheck(3) == 1) && ~isempty(surfX_RFR) && ~isempty(surfY_RFR)
                            for i = 1:size(currentPts,1)
                                currentPts(i,1) = currentPts(i,1) - surfX_RFR(currentPts(i,1)+xShift,currentPts(i,2)+yShift);
                                currentPts(i,2) = currentPts(i,2) - surfY_RFR(currentPts(i,1)+xShift,currentPts(i,2)+yShift);
                            end
                            farRedSurfCalTrue = true;
                            farRedAffineCalTrue = false;
                    ...
                    if ~isempty(surfaceOpt) && ~isempty(affineOpt) && (surfaceOpt == 1) && (affineOpt == 0)
                        if (channelCheck(1) == 1) && (channelCheck(3) == 1) && ~isempty(surfX_GFR) && ~isempty(surfY_GFR) && ~isempty(surfZ_GFR)
                            for i = 1:size(currentPts,1)
                                currentPts(i,1) = currentPts(i,1) - surfX_GFR(currentPts(i,1)+xShift,currentPts(i,2)+yShift);
                                currentPts(i,2) = currentPts(i,2) - surfY_GFR(currentPts(i,1)+xShift,currentPts(i,2)+yShift);
                                currentPts(i,3) = currentPts(i,3) - surfZ_GFR(currentPts(i,1)+xShift,currentPts(i,2)+yShift);
                            end
                            farRedSurfCalTrue = true;
                            farRedAffineCalTrue = false;
                        ...
                        elseif (channelCheck(1) == 0) && (channelCheck(2) == 1) && (channelCheck(3) == 1) && ~isempty(surfX_RFR) && ~isempty(surfY_RFR) && ~isempty(surfZ_RFR)
                            for i = 1:size(currentPts,1)
                                currentPts(i,1) = currentPts(i,1) - surfX_RFR(currentPts(i,1)+xShift,currentPts(i,2)+yShift);
                                currentPts(i,2) = currentPts(i,2) - surfY_RFR(currentPts(i,1)+xShift,currentPts(i,2)+yShift);
                                currentPts(i,3) = currentPts(i,3) - surfZ_RFR(currentPts(i,1)+xShift,currentPts(i,2)+yShift);
                            end
                            farRedSurfCalTrue = true;
                            farRedAffineCalTrue = false;
                ...
                if g2DOpt == 1
                    ...
                    if ~isempty(surfaceOpt) && ~isempty(affineOpt) && (surfaceOpt == 1) && (affineOpt == 0)
                        if (channelCheck(1) == 1) && (channelCheck(4) == 1) && ~isempty(surfX_GB) && ~isempty(surfY_GB)
                            for i = 1:size(currentPts,1)
                                currentPts(i,1) = currentPts(i,1) - surfX_GB(currentPts(i,1)+xShift,currentPts(i,2)+yShift);
                                currentPts(i,2) = currentPts(i,2) - surfY_GB(currentPts(i,1)+xShift,currentPts(i,2)+yShift);
                            end
                        ...
                        elseif (channelCheck(1) == 0) && (channelCheck(2) == 1) && (channelCheck(4) == 1) && ~isempty(surfX_RB) && ~isempty(surfY_RB)
                            for i = 1:size(currentPts,1)
                                currentPts(i,1) = currentPts(i,1) - surfX_RB(currentPts(i,1)+xShift,currentPts(i,2)+yShift);
                                currentPts(i,2) = currentPts(i,2) - surfY_RB(currentPts(i,1)+xShift,currentPts(i,2)+yShift);
                            end
                            blueSurfCalTrue = true;
                            blueAffineCalTrue = false;
                        ...
                        elseif (channelCheck(1) == 0) && (channelCheck(2) == 0) && (channelCheck(3) == 1) && (channelCheck(4) == 1) && ~isempty(surfX_FRB) && ~isempty(surfY_FRB)
                            for i = 1:size(currentPts,1)
                                currentPts(i,1) = currentPts(i,1) - surfX_FRB(currentPts(i,1)+xShift,currentPts(i,2)+yShift);
                                currentPts(i,2) = currentPts(i,2) - surfY_FRB(currentPts(i,1)+xShift,currentPts(i,2)+yShift);
                            end
                            blueSurfCalTrue = true;
                            blueAffineCalTrue = false;
                    ...
                    if ~isempty(surfaceOpt) && ~isempty(affineOpt) && (surfaceOpt == 1) && (affineOpt == 0)
                        if (channelCheck(1) == 1) && (channelCheck(4) == 1) && ~isempty(surfX_GB) && ~isempty(surfY_GB) && ~isempty(surfZ_GB)
                            for i = 1:size(currentPts,1)
                                currentPts(i,1) = currentPts(i,1) - surfX_GB(currentPts(i,1)+xShift,currentPts(i,2)+yShift);
                                currentPts(i,2) = currentPts(i,2) - surfY_GB(currentPts(i,1)+xShift,currentPts(i,2)+yShift);
                                currentPts(i,3) = currentPts(i,3) - surfZ_GB(currentPts(i,1)+xShift,currentPts(i,2)+yShift);
                            end
                            blueSurfCalTrue = true;
                            blueAffineCalTrue = false;
                        ...
                        elseif (channelCheck(1) == 0) && (channelCheck(2) == 1) && (channelCheck(4) == 1) && ~isempty(surfX_RB) && ~isempty(surfY_RB) && ~isempty(surfZ_RB)
                            for i = 1:size(currentPts,1)
                                currentPts(i,1) = currentPts(i,1) - surfX_RB(currentPts(i,1)+xShift,currentPts(i,2)+yShift);
                                currentPts(i,2) = currentPts(i,2) - surfY_RB(currentPts(i,1)+xShift,currentPts(i,2)+yShift);
                                currentPts(i,3) = currentPts(i,3) - surfZ_RB(currentPts(i,1)+xshift,currentPts(i,2)+yShift);
                            end
                            blueSurfCalTrue = true;
                            blueAffineCalTrue = false;
                        ...
                        elseif (channelCheck(1) == 0) && (channelCheck(2) == 0) && (channelCheck(3) == 1) && (channelCheck(4) == 1) && ~isempty(surfX_FRB) && ~isempty(surfY_FRB) && ~isempty(surfZ_FRB)
                            for i = 1:size(currentPts,1)
                                currentPts(i,1) = currentPts(i,1) - surfX_FRB(currentPts(i,1)+xShift,currentPts(i,2)+yShift);
                                currentPts(i,2) = currentPts(i,2) - surfY_FRB(currentPts(i,1)+xShift,currentPts(i,2)+yShift);
                                currentPts(i,3) = currentPts(i,3) - surfY_FRB(currentPts(i,1)+xShift,currentPts(i,2)+yShift);
                            end
                            blueSurfCalTrue = true;
                            blueAffineCalTrue = false;
...
function [fitParams] = fit3DGauss(particleBB, bbCoords, boxPts)
    ...
    global xyRes zRes fwhmDiv cropROI sortFittedData
...
function [fitParams] = fit2DGauss(particleBB, bbCoords, boxPts)
    ...
    global xyRes fwhmDiv sortFittedData
...
function exportData(~, ~)
    ...
    answerFOV = questdlg('Would you like to export Field Dependence Plots?', ...
        'Export Field Dependence Plots?', ...
        'Yes','No','Cancel','Yes');
    % Handle response
    switch answerFOV
        case 'Yes'
    ...
    % Save Field Dependence FOV Plots %
    
    if ~isempty(fovPlots) && (fovPlots == 1)
        
        imgExportPath = fullfile(folderExp,newDirName,'Field Dependence Plots');
        mkdir(imgExportPath);
        
        if ~isempty(finalFit488)
            disp('Saving Green Field Dependence Plots...');
            dataPts = finalFit488;
            if size(dataPts,2) == (gauss3DN + 1)
                % Collect Data %
                dataPts = dataPts(dataPts(:,24) == 0,:);
                dataPts = dataPts(~isoutlier(dataPts(:,15)),:);
                dataPts = dataPts(~isoutlier(dataPts(:,16)),:);
                dataPts = dataPts(~isoutlier(dataPts(:,12)),:);
                dataPts = dataPts(~isoutlier(dataPts(:,13)),:);
                dataPts = dataPts(~isoutlier(dataPts(:,14)),:);
                dataPts = dataPts(~isoutlier(dataPts(:,17)),:);
                dataPts = dataPts(~isoutlier(dataPts(:,18)),:);
                dataPts = dataPts(~isoutlier(dataPts(:,19)),:);
                dataPts = dataPts(~isoutlier(dataPts(:,20)),:);
                dataPts = dataPts(~isoutlier(dataPts(:,21)),:);
                dataPts = dataPts(~isoutlier(dataPts(:,22)),:);
                
                dataPts(any(isnan(dataPts),2), :) = [];
                
                tempInt = [dataPts(:,1) dataPts(:,2), dataPts(:,15)];
                
                tempMax = [dataPts(:,1) dataPts(:,2), dataPts(:,16)];
                
                tempGFWHM_X = [dataPts(:,1) dataPts(:,2), dataPts(:,12)];
                
                tempGFWHM_Y = [dataPts(:,1) dataPts(:,2), dataPts(:,13)];
                
                tempGFWHM_Z = [dataPts(:,1) dataPts(:,2), dataPts(:,14)];
                
                tempTFWHM_X = [dataPts(:,1) dataPts(:,2), dataPts(:,17)];
                
                tempTFWHM_Y = [dataPts(:,1) dataPts(:,2), dataPts(:,18)];
                
                tempTFWHM_Z = [dataPts(:,1) dataPts(:,2), dataPts(:,19)];
                
                tempIFWHM_X = [dataPts(:,1) dataPts(:,2), dataPts(:,20)];
                
                tempIFWHM_Y = [dataPts(:,1) dataPts(:,2), dataPts(:,21)];
                
                tempIFWHM_Z = [dataPts(:,1) dataPts(:,2), dataPts(:,22)];
                ...
                hold on;
                subplot(1,2,1);
                [fitSurfaceZ,gofZ] = fit([tempInt(:,1),tempInt(:,2)],tempInt(:,3), 'poly22');
                plot(fitSurfaceZ,[tempInt(:,1),tempInt(:,2)], tempInt(:,3))
                set(gca, 'FontSize', fontSizeAxes);
                title('Integrated Intensity', 'FontSize', fontSize);
                colorbar;
                view(0,90)
                
                subplot(1,2,2);
                [fitSurfaceZ,gofZ] = fit([tempMax(:,1),tempMax(:,2)],tempMax(:,3), 'poly22');
                plot(fitSurfaceZ,[tempMax(:,1),tempMax(:,2)], tempMax(:,3))
                set(gca, 'FontSize', fontSizeAxes);
                title('Maximum Intensity', 'FontSize', fontSize);
                colorbar;
                view(0,90)
                ...
                subplot(3,3,1);
                [fitSurfaceZ,gofZ] = fit([tempGFWHM_X(:,1),tempGFWHM_X(:,2)],tempGFWHM_X(:,3), 'poly22');
                plot(fitSurfaceZ,[tempGFWHM_X(:,1),tempGFWHM_X(:,2)], tempGFWHM_X(:,3))
                set(gca, 'FontSize', fontSizeAxes);
                ylabel('Gaussian FWHM', 'FontSize', fontSize);
                title('X FWHM', 'FontSize', fontSize);
                colorbar;
                view(0,90)
                
                subplot(3,3,2);
                [fitSurfaceZ,gofZ] = fit([tempGFWHM_Y(:,1),tempGFWHM_Y(:,2)],tempGFWHM_Y(:,3), 'poly22');
                plot(fitSurfaceZ,[tempGFWHM_Y(:,1),tempGFWHM_Y(:,2)], tempGFWHM_Y(:,3))
                set(gca, 'FontSize', fontSizeAxes);
                title('Y FWHM', 'FontSize', fontSize);
                colorbar;
                view(0,90)
                
                subplot(3,3,3);
                [fitSurfaceZ,gofZ] = fit([tempGFWHM_Z(:,1),tempGFWHM_Z(:,2)],tempGFWHM_Z(:,3), 'poly22');
                plot(fitSurfaceZ,[tempGFWHM_Z(:,1),tempGFWHM_Z(:,2)],tempGFWHM_Z(:,3))
                set(gca, 'FontSize', fontSizeAxes);
                title('Z FWHM', 'FontSize', fontSize);
                colorbar;
                view(0,90)
                
                
                subplot(3,3,4);
                [fitSurfaceZ,gofZ] = fit([tempTFWHM_X(:,1),tempTFWHM_X(:,2)],tempTFWHM_X(:,3), 'poly22');
                plot(fitSurfaceZ,[tempTFWHM_X(:,1),tempTFWHM_X(:,2)], tempTFWHM_X(:,3))
                set(gca, 'FontSize', fontSizeAxes);
                ylabel('True FWHM', 'FontSize', fontSize);
                colorbar;
                view(0,90)
                
                subplot(3,3,5);
                [fitSurfaceZ,gofZ] = fit([tempTFWHM_Y(:,1),tempTFWHM_Y(:,2)],tempTFWHM_Y(:,3), 'poly22');
                plot(fitSurfaceZ,[tempTFWHM_Y(:,1),tempTFWHM_Y(:,2)], tempTFWHM_Y(:,3))
                set(gca, 'FontSize', fontSizeAxes);
                colorbar;
                view(0,90)
                
                subplot(3,3,6);
                [fitSurfaceZ,gofZ] = fit([tempTFWHM_Z(:,1),tempTFWHM_Z(:,2)],tempTFWHM_Z(:,3), 'poly22');
                plot(fitSurfaceZ,[tempTFWHM_Z(:,1),tempTFWHM_Z(:,2)],tempTFWHM_Z(:,3))
                set(gca, 'FontSize', fontSizeAxes);
                colorbar;
                view(0,90)
                
                subplot(3,3,7);
                [fitSurfaceZ,gofZ] = fit([tempIFWHM_X(:,1),tempIFWHM_X(:,2)],tempIFWHM_X(:,3), 'poly22');
                plot(fitSurfaceZ,[tempIFWHM_X(:,1),tempIFWHM_X(:,2)], tempIFWHM_X(:,3))
                set(gca, 'FontSize', fontSizeAxes);
                ylabel('Interpolated FWHM', 'FontSize', fontSize);
                colorbar;
                view(0,90)
                
                subplot(3,3,8);
                [fitSurfaceZ,gofZ] = fit([tempIFWHM_Y(:,1),tempIFWHM_Y(:,2)],tempIFWHM_Y(:,3), 'poly22');
                plot(fitSurfaceZ,[tempIFWHM_Y(:,1),tempIFWHM_Y(:,2)], tempIFWHM_Y(:,3))
                set(gca, 'FontSize', fontSizeAxes);
                colorbar;
                view(0,90)
                
                subplot(3,3,9);
                [fitSurfaceZ,gofZ] = fit([tempIFWHM_Z(:,1),tempIFWHM_Z(:,2)],tempIFWHM_Z(:,3), 'poly22');
                plot(fitSurfaceZ,[tempIFWHM_Z(:,1),tempIFWHM_Z(:,2)],tempIFWHM_Z(:,3))
                set(gca, 'FontSize', fontSizeAxes);
                colorbar;
                view(0,90)
                
            elseif size(dataPts,2) == (gauss2DN + 1)
                % Collect Data %
                dataPts = dataPts(dataPts(:,17) == 0,:);
                
                dataPts = dataPts(~isoutlier(dataPts(:,10)),:);
                dataPts = dataPts(~isoutlier(dataPts(:,11)),:);
                dataPts = dataPts(~isoutlier(dataPts(:,8)),:);
                dataPts = dataPts(~isoutlier(dataPts(:,9)),:);
                dataPts = dataPts(~isoutlier(dataPts(:,12)),:);
                dataPts = dataPts(~isoutlier(dataPts(:,13)),:);
                dataPts = dataPts(~isoutlier(dataPts(:,14)),:);
                dataPts = dataPts(~isoutlier(dataPts(:,15)),:);
                ...
                tempInt = [dataPts(:,1) dataPts(:,2), dataPts(:,10)];
                
                tempMax = [dataPts(:,1) dataPts(:,2), dataPts(:,11)];
                
                tempGFWHM_X = [dataPts(:,1) dataPts(:,2), dataPts(:,8)];
                
                tempGFWHM_Y = [dataPts(:,1) dataPts(:,2), dataPts(:,9)];
                
                tempTFWHM_X = [dataPts(:,1) dataPts(:,2), dataPts(:,12)];
                
                tempTFWHM_Y = [dataPts(:,1) dataPts(:,2), dataPts(:,13)];
                
                tempIFWHM_X = [dataPts(:,1) dataPts(:,2), dataPts(:,14)];
                
                tempIFWHM_Y = [dataPts(:,1) dataPts(:,2), dataPts(:,15)];
            ...
            if gDim == 3
                saveas(intensityFig, fullfile(imgExportPath,strcat(fname,'_3D_IntensityPlots_Green')));
                saveas(fwhmFig, fullfile(imgExportPath,strcat(fname,'_3D_FWHMPlots_Green')));
            elseif gDim == 2
                saveas(intensityFig, fullfile(imgExportPath,strcat(fname,'_2D_IntensityPlots_Green')));
                saveas(fwhmFig, fullfile(imgExportPath,strcat(fname,'_2D_FWHMPlots_Green')));
            end
        ...
        if ~isempty(finalFit594)
            disp('Saving Red Field Dependence Plots...');
            dataPts = finalFit594;
            if size(dataPts,2) == (gauss3DN + 1)
                % Collect Data %
                dataPts = dataPts(dataPts(:,24) == 0,:);
            ...
            if gDim == 3
                saveas(intensityFig, fullfile(imgExportPath,strcat(fname,'_3D_IntensityPlots_Red')));
                saveas(fwhmFig, fullfile(imgExportPath,strcat(fname,'_3D_FWHMPlots_Red.tif')));
            elseif gDim == 2
                saveas(intensityFig, fullfile(imgExportPath,strcat(fname,'_2D_IntensityPlots_Red')));
                saveas(fwhmFig, fullfile(imgExportPath,strcat(fname,'_2D_FWHMPlots_Red.tif')));
            end
            close(intensityFig);
            close(fwhmFig);
        end
        ...
        if ~isempty(finalFit640)
            disp('Saving Far Red Field Dependence Plots...');
            dataPts = finalFit640;
            if size(dataPts,2) == (gauss3DN + 1)
                % Collect Data %
                dataPts = dataPts(dataPts(:,24) == 0,:);
            ...
            if gDim == 3
                saveas(intensityFig, fullfile(imgExportPath,strcat(fname,'_3D_IntensityPlots_FarRed')));
                saveas(fwhmFig, fullfile(imgExportPath,strcat(fname,'_3D_FWHMPlots_FarRed.tif')));
            elseif gDim == 2
                saveas(intensityFig, fullfile(imgExportPath,strcat(fname,'_2D_IntensityPlots_FarRed')));
                saveas(fwhmFig, fullfile(imgExportPath,strcat(fname,'_2D_FWHMPlots_FarRed.tif')));
            end
            close(intensityFig);
            close(fwhmFig);
        end
        ...
        if ~isempty(finalFit405)
            disp('Saving Blue Field Dependence Plots...');
            dataPts = finalFit405;
            if size(dataPts,2) == (gauss3DN + 1)
                % Collect Data %
                dataPts = dataPts(dataPts(:,24) == 0,:);
            ...
            if gDim == 3
                saveas(intensityFig, fullfile(imgExportPath,strcat(fname,'_3D_IntensityPlots_Blue')));
                saveas(fwhmFig, fullfile(imgExportPath,strcat(fname,'_3D_FWHMPlots_Blue.tif')));
            elseif gDim == 2
                saveas(intensityFig, fullfile(imgExportPath,strcat(fname,'_2D_IntensityPlots_Blue')));
                saveas(fwhmFig, fullfile(imgExportPath,strcat(fname,'_2D_FWHMPlots_Blue.tif')));
            end
            close(intensityFig);
            close(fwhmFig);
        end
...
function [intensityFigF, fwhmFigF] = getFOVPlots(dataPts)

    global gauss3DN gauss2DN
...
function [sortedData] = sortData(fitParams)

    global gauss2DN gauss3DN
    
    if size(fitParams,2) == gauss3DN
        ...
        [M,maxInd] = max([tX tY tZ]);
        [M,minInd] = min([tX tY tZ]);
        
        if (maxInd == 2) && (minInd == 1)
    ...
    elseif size(fitParams,2) == gauss2DN
